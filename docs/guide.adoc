...
`enable-count-based-layouts = true` (disabled by default) makes every workspace's tiling tree deterministic by tiling-window count:

----
2 windows   [ A | B ]            side by side
3 windows   [ A | (B over C) ]   one primary + a stack of 2 (stack on either side is accepted)
4 windows   [ (A over B) | (C over D) ]   2x2 grid
1 and 5+    untouched
----

Floating windows, popups, minimized and macOS-fullscreen windows don't participate in the count.
On a vertical root container the layout transposes.
The reshape preserves window order and only fires when the current shape doesn't match, so manual resizes survive.
