...
final class PersistedWorkspaceStateTest: XCTestCase {
    ...
    func testJsonRoundTripPreservesWorldAndRecomputesWindowIds() throws {
        let persisted = PersistedWorld(makeFrozenWorld(), bootTime: 1000, focusedWorkspace: "3")
        ...
        assertEquals(decoded.focusedWorkspace, "3")
        ...
        let reencoded = try encode(PersistedWorld(restored, bootTime: 1000, focusedWorkspace: "3"))
        assertEquals(reencoded, data)
    }

    func testFileWithoutFocusedWorkspaceStillDecodes() throws {
        // Files written by fork.6 predate the focusedWorkspace field
        let persisted = PersistedWorld(makeFrozenWorld(), bootTime: 1000, focusedWorkspace: nil)
        let data = try encode(persisted)
        XCTAssertFalse(String(data: data, encoding: .utf8).orDie().contains("focusedWorkspace"))
        let decoded = try JSONDecoder().decode(PersistedWorld.self, from: data)
        assertNil(decoded.focusedWorkspace)
        XCTAssertNotNil(decoded.toFrozenWorld())
    }
