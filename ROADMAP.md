...
## Simulator centering thrash (user-reported 2026-08-03, deployed in v0.21.3-fork.18)

- [x] Branch `fix/simulator-center-thrash` — the iOS Simulator "freaked out trying to
      center no matter what". Root cause: once a window was classified clamping, every
      layout pass STILL requested the full tile size (setAxFrame(centered, target)).
      System Settings ignores that silently; the Simulator aspect-fits, so each oversize
      request kicked off its own re-fit, the post-write readback caught a shifted size,
      the clamp cache churned (v4's `clamping(actual)` update), the window re-centered,
      and the move/resize events triggered the next pass - a visible feedback loop.
      Fix: steady state is position-only (no size request a confirmed-clamping window
      would refuse); self-sized changes (rotation, Settings pane switch) are detected by
      a READ-ONLY drift probe (single-observation update is safe there - nothing was
      requested, so no async-resize race, unlike the Chrome v3 lesson); the clamp is
      cached per tile TARGET, and a changed tile resets classification so aspect-fitting
      windows can scale up to bigger tiles via the normal 3-confirmation re-probe.
      Live-validated on the deployed build: Simulator on a visible workspace settled at
      the exact tile center (456x972 at 1066+89 in an 852x1067 tile) and stayed
      byte-identical across 6s of 0.5s samples; one transient top-left placement during
      post-restart re-classification is by design. No clamp-behavior unit tests exist
      (MacWindow+AX only, same as v1-v4); suite green at 422.
...
- MacBook: fork v0.21.3-fork.18 live (fork.17 + Simulator centering-thrash fix; fork.17 = fork.16 + 3-window layout semantics: mirror/
...
- Mac mini: fork v0.21.3-fork.18 (was fork.14 — the 2026-07-30 "stock Homebrew" note
